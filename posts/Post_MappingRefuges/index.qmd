...
if(params$user %in% 'KateWork'){
  ...
  # set input
  dataloc = 'C:/Users/khayes23/Documents/Research/R00 Admin/R030 Website/krhayes.com/posts/Post_MappingRefuges'
...
alaska = vect(paste0(dataloc, "/data/AK_boundary.shp"))
...
ggplot() +
  geom_spatvector(data = alaska, fill = "grey", alpha = 0.75, col = "grey")+
  theme_minimal()
```
...
```{r}
refuge = vect(paste0(dataloc, "/data/Boundaries_Refuge.shp"))
crs(refuge) = crs(alaska) # check that it has the same projection

ggplot() +
  geom_spatvector(data = alaska, fill = "grey",col = "grey", alpha = 0.75) +
...
```{r}
alaska_hires = vect(paste0(dataloc, "/data/AK_hires.shp"))
