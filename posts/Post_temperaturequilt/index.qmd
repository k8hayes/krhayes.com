...
In my free time, I'm a very amateur fiber arts nerd. Even calling it "fiber arts" makes me cringe. I used to sew some of my own clothes (mostly during covid), and knit often on zoom calls and in meetings. Technically, I've made a quilt before.
...
From there it was a hop and a skip over to temperature quilts - a craft tradition where each day (or month) of the year gets a color based on the temperature, and you knit or sew them together into a record of the year's weather.
...
if(params$user %in% 'KateWork'){
  ...
  # set input
  dataloc = paste0(here(), "/posts/Post_temperaturequilt/data")
...
ggplot(triangle, aes(x = polygon.x, y = polygon.y)) + geom_polygon(fill = "#f46d43")
```
...
ggplot(left.triangle, aes(x = polygon.x, y = polygon.y)) + geom_polygon( fill = "#313695")
```
...
ggplot(both.triangle) + 
  geom_polygon(aes(x = polygon.x, y = polygon.y, 
                   group = triangle, fill = triangle)) + 
  scale_fill_manual(values = c("#313695","#f46d43"))
```
...
quilt_plot = ggplot(quilt, aes(x = polygon.x, y = polygon.y,
...
```{r}
save_plot("businesscard_quilt.png", quilt_plot, base_height = 3.71, base_aspect_ratio = 2.1) 
```
